...
def safe_print(msg):
    """Print message with proper encoding handling for Windows console."""
    try:
        print(msg)
    except UnicodeEncodeError:
        # Fallback to ASCII-compatible version
        print(msg.encode('ascii', 'replace').decode('ascii'))


def select_torch_install_plan(accel_pref: str, platform: str, has_nvidia: bool):
    """Return a plan dict for how to install torch based on accelerator preference.

    Returns one of:
    - {'mode': 'cuda', 'targets': ['cu121','cu118','cpu']}
    - {'mode': 'amd-directml'}
    - {'mode': 'amd-rocm', 'indices': [<rocm index urls>]}
    - {'mode': 'cpu'}
    """
    pref = (accel_pref or "auto").lower()
    if pref not in {"auto", "cuda", "amd", "cpu"}:
        pref = "auto"
    is_windows = platform.startswith("win")

    if pref == "cuda" or (pref == "auto" and has_nvidia):
        return {"mode": "cuda", "targets": ["cu121", "cu118", "cpu"]}

    if pref == "amd" or (pref == "auto" and not has_nvidia):
        if is_windows:
            return {"mode": "amd-directml"}
        else:
            return {
                "mode": "amd-rocm",
                "indices": [
...
def install_dependencies():
    ...
    # Skip installation for PyInstaller executables
    if getattr(sys, 'frozen', False):
        safe_print("[INFO] Running from executable - skipping dependency checks")
        return True  # Return True to continue without ML features
    ...
    for package in required_packages:
        try:
            if package == "Pillow":
                ...
                __import__(package)
            safe_print(f"[OK] {package} - OK")
        except ImportError:
            missing_packages.append(package)
            safe_print(f"[MISSING] {package} - MISSING")

    # Helper: detect NVIDIA GPU by checking nvidia-smi
    ...
    # Determine accelerator mode (auto | cuda | amd | cpu)
    accel_pref = os.environ.get("PHOTOFILTER_TORCH_ACCELERATOR", "auto").lower()
    if accel_pref not in {"auto", "cuda", "amd", "cpu"}:
        accel_pref = "auto"
    logging.info(f"PHOTOFILTER_TORCH_ACCELERATOR={accel_pref}")

    if missing_packages:
        safe_print(f"[*] Installing {len(missing_packages)} missing packages...")

        for package in missing_packages:
            try:
                safe_print(f"[*] Installing {package}...")
                if package == "torch":
                    # Decide desired target and attempt sequence
                    wants_cuda = (accel_pref == "cuda") or (
                        accel_pref == "auto" and _has_nvidia_gpu()
                    )
                    is_windows = sys.platform.startswith("win")

                    # AMD path: prefer DirectML on Windows, ROCm on Linux
                    if (accel_pref == "amd") or (
                        accel_pref == "auto" and not wants_cuda
                    ):
                        if is_windows:
                            # DirectML backend for Windows/AMD GPUs
                            try:
                                safe_print("[*] Attempting torch-directml (Windows/AMD)...")
                                ...
                                safe_print("[OK] Installed torch-directml")
                                # Also install CPU torch/torchvision wheels to satisfy base imports
                                print(
                                    "➡️  Installing torch/torchvision CPU wheels for DirectML..."
                                )
                                subprocess.check_call(
                                    [
                                        sys.executable,
                                ...
                                safe_print(
                                    "[OK] Installed torch/torchvision (CPU) for DirectML"
                                )
                                continue
                            except subprocess.CalledProcessError:
                                safe_print(
                                    "[ERROR] Failed to install torch-directml or base CPU wheels"
                                )
                                # Fallback to CPU wheels
                                pass
                        else:
                            # Try ROCm wheels on Linux
                            index_map_rocm = [
                                "https://download.pytorch.org/whl/rocm6.0",
                                "https://download.pytorch.org/whl/rocm5.6",
                            ]
                            rocm_ok = False
                            for idx in index_map_rocm:
                                try:
                                    safe_print(
                                        f"[*] Attempting PyTorch ROCm install from {idx}..."
                                    )
                                    subprocess.check_call(
                                        [
                                            sys.executable,
                                    ...
                                    safe_print("[OK] Installed torch/torchvision (ROCm)")
                                    rocm_ok = True
                                    break
                                except subprocess.CalledProcessError:
                                    safe_print(f"[ERROR] Failed ROCm install from {idx}")
                                    continue
                            if rocm_ok:
                                continue
                            # Fall through to CPU if ROCm unavailable

                    # CUDA or CPU path
                    attempt_targets = (
                        ["cu121", "cu118", "cpu"] if wants_cuda else ["cpu"]
                    )
                    index_map = {
                    ...
                    for target in attempt_targets:
                        try:
                            safe_print(f"[*] Attempting PyTorch install for {target}...")
                            subprocess.check_call(
                                [
                                    sys.executable,
                                    "-m",
                            ...
                            installed = True
                            safe_print(f"[OK] Installed torch/torchvision for {target}")
                            break
                        except subprocess.CalledProcessError:
                            safe_print(f"[ERROR] Failed torch/torchvision install for {target}")
                            continue

                    if not installed:
                        return False
                elif package == "torchvision":
                    # Ensure torchvision matches existing torch build
                    target = "cpu"
                    try:
                        import torch as _t

                        # If torch was compiled with CUDA, prefer its matching wheel index
                        compiled_cuda = getattr(_t.version, "cuda", None)
                        if compiled_cuda:
                            if compiled_cuda.startswith("12.1"):
                                target = "cu121"
                            elif compiled_cuda.startswith("11.8"):
                                target = "cu118"
                        # If runtime CUDA available, we also prefer CUDA
                        if hasattr(_t, "cuda") and _t.cuda.is_available():
                            # Prefer cu121 first, then fallback
                            target = "cu121" if target != "cu118" else "cu118"
                    except Exception:
                        target = "cpu"
                    ...
                    attempts = [target] if target != "cpu" else ["cpu"]
                    if target == "cu121":
                        attempts = ["cu121", "cu118", "cpu"]
                    elif target == "cu118":
                        attempts = ["cu118", "cu121", "cpu"]

                    installed = False
                    for tgt in attempts:
                        try:
                            safe_print(f"[*] Installing torchvision for {tgt}...")
                            subprocess.check_call(
                                [
                                    sys.executable,
                                    "-m",
                            ...
                            installed = True
                            safe_print(f"[OK] torchvision installed for {tgt}")
                            break
                        except subprocess.CalledProcessError:
                            safe_print(f"[ERROR] Failed torchvision install for {tgt}")
                            continue
    ...
    print("[OK] All dependencies already available")
    return True
...
try:
    ...
    GUI_AVAILABLE = True
    print("[OK] GUI libraries available")
except ImportError as e:
    print(f"[ERROR] GUI libraries missing: {e}")
    GUI_AVAILABLE = False
...
try:
    ...
    # Use importlib to dynamically import modules - hides from PyInstaller static analysis
    import importlib
    recognition_module = importlib.import_module("photofilter.core.recognition")
    detectors_module = importlib.import_module("photofilter.core.detectors")

    PhotoOrganizer = recognition_module.PhotoOrganizer
    YOLOv5Detector = detectors_module.YOLOv5Detector
    CORE_AVAILABLE = True
    print("[OK] Core recognition modules available")
except ImportError as e:
    print(f"[WARN] Core modules not available: {e}")
    print("This is normal if dependencies aren't installed yet")
    logging.warning(f"Core modules import failed: {e}")
...
try:
    import importlib
    deduplication_module = importlib.import_module("photofilter.core.deduplication")

    DeduplicationEngine = deduplication_module.DeduplicationEngine
    DEDUPLICATION_AVAILABLE = True
    print("[OK] Deduplication engine available")
except ImportError as e:
    print(f"[WARN] Deduplication engine not available: {e}")
    print("This is normal if dependencies aren't installed yet")
    logging.warning(f"Deduplication module import failed: {e}")
...
try:
    ...
    AVAILABLE_DETECTORS = detectors_module.AVAILABLE_DETECTORS
    ADVANCED_DETECTORS_AVAILABLE = True
    print("[OK] Advanced detectors available")
except ImportError:
    print("[WARN] Advanced detectors not available - using basic mode")
    logging.info("Advanced detectors not available - falling back to basic mode")
...
class DetectionCard(ttk.Frame):
    ...
    def __init__(self, parent, on_selection_change=None):
        ...
        try:
            self.create_widgets()
            safe_print("[OK] DetectionCard widgets created successfully")
        except Exception as e:
            safe_print(f"[ERROR] Error creating DetectionCard widgets: {e}")
    ...
    def create_category_tabs(self):
        ...
        # Build a normalized view of currently selected items to restore state across toggles
        selected_normalized = set(
            self.ALIAS_MAP.get(s.lower(), s.lower()) for s in self.selected_objects
        )
        ...
        for category, objects in objects_source.items():
            ...
            for obj in objects:
                ...
                # Match by canonical normalized value so aliases map to canonical during toggle
                obj_norm = self.ALIAS_MAP.get(obj.lower(), obj.lower())
                if (obj in self.selected_objects) or (obj_norm in selected_normalized):
                    var.set(True)
...
class PhotoRecognitionGUI:
    ...
    def __init__(self):
        safe_print("[*] Initializing PhotoRecognitionGUI...")
        ...
        try:
            ...
            safe_print("[OK] PhotoRecognitionGUI initialized successfully")

        except Exception as e:
            safe_print(f"[ERROR] Error initializing GUI: {e}")
            raise
    ...
    def create_widgets(self):
        ...
        try:
            ...
            safe_print("[OK] Optimized single-column GUI layout created")

        except Exception as e:
            safe_print(f"[ERROR] Error creating widgets: {e}")
            self.create_fallback_gui()

    def create_fallback_gui(self):
        """Create basic GUI if main creation fails."""
        safe_print("[WARN] Creating fallback GUI due to initialization error")
        ...
        safe_print("[OK] Fallback GUI created successfully")
    ...
    def create_optimized_detection_section(self, parent):
        ...
        # Set default detection methods to only check filenames and filesizes
        self.dedup_check_filenames = tk.BooleanVar(value=True)  # Enabled by default
        self.dedup_check_filesizes = tk.BooleanVar(value=True)  # Enabled by default
        self.dedup_check_metadata = tk.BooleanVar(value=False)  # Disabled by default
        self.dedup_check_visual = tk.BooleanVar(value=False)  # Disabled by default
        ...
        # Reordered actions with 'Move to Organized Folders' second
        ttk.Radiobutton(
            action_frame,
    ...
    def check_and_display_capabilities(self):
        ...
        if not DEDUPLICATION_AVAILABLE:
            safe_print("   [INFO] To enable deduplication, install: pip install imagehash")

        safe_print("[OK] Application Ready!\n")
    ...
    def start_deduplication_processing(self):
        """Start the deduplication process in a separate thread."""
        try:
            ...
            if hasattr(self, "clear_deduplication_results"):
                self.clear_deduplication_results()

            if hasattr(self, "current_duplicate_groups"):
                self.current_duplicate_groups = []
    ...
    def process_deduplication(self):
        """Process deduplication in background thread with file movement support."""
        try:
            ...
            # Get the action from the UI
            action = "preview_only"
            if hasattr(self, "dedup_action_var"):
                action = self.dedup_action_var.get()

            # Get the output folder if moving to a specific location
            output_folder = None
            if (
                action == "move_to_folder"
                and hasattr(self, "folder_entry")
                and self.folder_entry.get()
            ):
                output_folder = Path(self.folder_entry.get())
                try:
                    ...
                    self.root.after(
                        0, self.handle_error, f"Cannot write to output folder: {str(e)}"
                    )
                    return

            # Create deduplication engine
            engine = DeduplicationEngine(threshold=self.dedup_threshold_var.get())
            engine.set_progress_callback(self.update_dedup_progress)

            # Configure detection methods from UI
            engine.check_filenames = (
                self.dedup_check_filenames.get()
                if hasattr(self, "dedup_check_filenames")
                else True
            )
            engine.check_filesizes = (
                self.dedup_check_filesizes.get()
                if hasattr(self, "dedup_check_filesizes")
                else True
            )
            engine.check_metadata = (
                self.dedup_check_metadata.get()
                if hasattr(self, "dedup_check_metadata")
                else False
            )
            engine.check_visual_similarity = (
                self.dedup_check_visual.get()
                if hasattr(self, "dedup_check_visual")
                else False
            )

            # Log the settings
            self.add_dedup_log(f"🔧 Action: {action}")
            if output_folder:
                self.add_dedup_log(f"🔧 Output folder: {output_folder}")
            self.add_dedup_log(f"🔧 Threshold: {engine.similarity_threshold}")
            self.add_dedup_log(f"🔧 Check filenames: {engine.check_filenames}")
            self.add_dedup_log(f"🔧 Check filesizes: {engine.check_filesizes}")
            self.add_dedup_log(f"🔧 Check metadata: {engine.check_metadata}")
            self.add_dedup_log(f"🔧 Check visual: {engine.check_visual_similarity}")
            ...
            if not source_path.exists() or not source_path.is_dir():
                self.root.after(
                    0, self.handle_error, f"Source folder does not exist: {source_path}"
                )
                return
            ...
            try:
                ...
                self.root.after(
                    0, self.handle_error, f"Error finding duplicates: {str(e)}"
                )
                return

            self.add_dedup_log(f"🔍 Found {len(duplicate_groups)} duplicate groups")
            self.add_dedup_log(
                f"📊 Total files analyzed: {getattr(engine, 'total_files_analyzed', 'N/A')}"
            )

            if (
                not self.is_processing or not duplicate_groups
            ):  # Check if cancelled or no duplicates
            ...
            if action == "preview_only":
                ...
                # Process the duplicates (move or organize)
                try:
                    moved_files = engine.remove_duplicates(
                        duplicate_groups,
                    ...
                    self.root.after(
                        0,
                        self.display_deduplication_results,
                        duplicate_groups,
                        engine,
                        moved_files,
                    )
                    ...
                    self.root.after(
                        0, self.handle_error, f"Error processing duplicates: {str(e)}"
                    )
                    return
    ...
    def display_deduplication_results(self, duplicate_groups, engine, moved_files=None):
        """Display deduplication results in the GUI.

        Args:
            duplicate_groups: List of duplicate file groups
            engine: DeduplicationEngine instance
            moved_files: Optional list of tuples (src, dst) of files that were moved/removed
        """
        try:
            # Clear previous results
            self.clear_deduplication_results()
            ...
            self.add_dedup_log(
                f"🎯 Found {len(duplicate_groups)} duplicate groups ({total_duplicates} duplicate files)"
            )

            # Show files analyzed and potential savings if available
            if hasattr(engine, "total_files_analyzed"):
                self.add_dedup_log(
                    f"📊 Total files analyzed: {engine.total_files_analyzed}"
                )

            if hasattr(engine, "total_duplicates_found"):
                self.add_dedup_log(
                    f"🔄 Total duplicates: {engine.total_duplicates_found}"
                )

            if hasattr(engine, "potential_savings") and engine.potential_savings > 0:
                self.add_dedup_log(
                    f"💾 Potential space savings: {engine.potential_savings / (1024*1024):.1f} MB"
                )

            # Show moved/removed files if any
            if moved_files:
                self.add_dedup_log("\n📂 Files moved/removed:")
                for src, dst in moved_files:
                    if dst:  # Moved to a new location
                        self.add_dedup_log(f"   • {src.name} → {dst}")
                    else:  # Deleted
                        self.add_dedup_log(f"   • {src.name} (removed)")

                # Calculate actual space saved
                try:
                    total_saved = sum(
                        src.stat().st_size for src, _ in moved_files if not dst
                    )
                    if total_saved > 0:
                        self.add_dedup_log(
                            f"\n💾 Actual space saved: {total_saved / (1024*1024):.1f} MB"
                        )
                except Exception as e:
            ...
            for i, group in enumerate(duplicate_groups, 1):
                ...
                self.add_dedup_log(
                    f"\n📁 Group {i} ({group_size} files, {group.duplicate_type}):"
                )

                # Show similarity score if available
                if hasattr(group, "similarity_score"):
                    self.add_dedup_log(f"   Similarity: {group.similarity_score:.1%}")

                # Show recommended action if available
                if hasattr(group, "recommended_action"):
                    self.add_dedup_log(
                ...
                for file_path in group.files:
                    try:
                        ...
                        status = ""
                        if moved_files and file_path in [src for src, _ in moved_files]:
                            status = " (moved/removed)"
                        self.add_dedup_log(
                            f"   • {file_path.name} ({file_size:.1f} MB){status}"
                        )
                    except Exception as e:
                        self.add_dedup_log(f"   • {file_path} (error: {str(e)})")

            # Show final summary
            self.add_dedup_log("\n✅ Deduplication complete!")

            # Enable action buttons
            if hasattr(self, "export_dedup_button"):
                self.export_dedup_button.config(state="normal")

            # Only enable apply button if we're in preview mode
            if (
                hasattr(self, "dedup_action_var")
                and self.dedup_action_var.get() == "preview_only"
                and hasattr(self, "apply_dedup_button")
            ):
    ...
    def load_settings(self):
        ...
        try:
            settings_path = Path.home() / ".photo_organizer_settings.json"
            if settings_path.exists():
                with open(settings_path, "r") as f:
                    settings = json.load(f)
                    print(f"Loaded settings: {settings}")  # Debug log

                    # Set source and output folders from settings if they exist
                    if "source_folder" in settings and settings["source_folder"]:
                        self.source_path.set(settings["source_folder"])
                        print(
                            f"Set source folder to: {settings['source_folder']}"
                        )  # Debug log
                    if "output_folder" in settings and settings["output_folder"]:
                        self.output_path.set(settings["output_folder"])
                        print(
                            f"Set output folder to: {settings['output_folder']}"
                        )  # Debug log

                    # Load deduplication settings if they exist
                    if "dedup_check_filenames" in settings:
                        self.dedup_check_filenames.set(
                            settings["dedup_check_filenames"]
                        )
                    if "dedup_check_filesizes" in settings:
                        self.dedup_check_filesizes.set(
                            settings["dedup_check_filesizes"]
                        )
                    if "dedup_check_metadata" in settings:
                        self.dedup_check_metadata.set(settings["dedup_check_metadata"])
                    if "dedup_check_visual" in settings:
                        self.dedup_check_visual.set(settings["dedup_check_visual"])
                    if "dedup_action" in settings:
                        self.dedup_action_var.set(settings["dedup_action"])
    ...
    def save_settings(self):
        ...
        try:
            ...
            settings = {
                ...
                "confidence_threshold": self.confidence_var.get(),
                "dedup_threshold": self.dedup_threshold_var.get(),
                "source_folder": self.source_path.get(),
                "output_folder": self.output_path.get(),
                "dedup_check_filenames": self.dedup_check_filenames.get(),
                "dedup_check_filesizes": self.dedup_check_filesizes.get(),
                "dedup_check_metadata": self.dedup_check_metadata.get(),
                "dedup_check_visual": self.dedup_check_visual.get(),
                "dedup_action": self.dedup_action_var.get(),
            }
    ...
    def on_closing(self):
        ...
        try:
            ...
            try:
                # Save current folder paths to settings
                self.settings["source_folder"] = self.source_path.get()
                self.settings["output_folder"] = self.output_path.get()
                self.save_settings()
            except Exception as e:
    ...
    def run(self):
        ...
        try:
            ...
            safe_print("[*] Starting GUI main loop...")
            self.root.mainloop()
        except Exception as e:
            safe_print(f"[ERROR] Error running GUI: {e}")


def main():
    """Main application entry point."""
    safe_print("\n" + "=" * 60)
    safe_print("PHOTO RECOGNITION GUI - PRODUCTION VERSION")
    safe_print("=" * 60)

    # Check GUI availability
    if not GUI_AVAILABLE:
        safe_print("[ERROR] GUI not available. Install tkinter:")
    ...
    # Skip dependency installation for PyInstaller executables
    if getattr(sys, 'frozen', False):
        safe_print("\n[INFO] Running from compiled executable - ML features disabled")
        safe_print("[INFO] This is a lightweight version without ML capabilities")
    else:
        # Install dependencies if needed (only for development)
        safe_print("\n[*] Checking dependencies...")
        if not install_dependencies():
            safe_print("[ERROR] Failed to install dependencies")
            safe_print("Manual installation required:")
            safe_print("   pip install torch torchvision ultralytics Pillow numpy")
            return
    ...
    if not CORE_AVAILABLE:
        try:
            # Ensure src is on sys.path and import from photofilter.core
            ...
            YOLOv5Detector = detectors_module.YOLOv5Detector
            CORE_AVAILABLE = True
            safe_print("[OK] Core modules loaded after dependency installation")
        except ImportError as e:
            safe_print(f"[WARN] Core modules still not available: {e}")
    ...
    try:
        ...
        safe_print("[OK] GUI created successfully!")
        safe_print("[*] All components should be visible and working")
        safe_print("[*] Features available:")
        safe_print("   - Object detection with predefined categories")
        safe_print("   - Custom object input and management")
        safe_print("   - Multiple AI model options")
        safe_print("   - Advanced photo deduplication")
        safe_print("   - Progress tracking and logging")
        safe_print("   - Folder configuration")
        safe_print("\n[*] GUI is now running - close the window to exit")

        app.run()

    except Exception as e:
        safe_print(f"[ERROR] GUI Error: {e}")
        import traceback
